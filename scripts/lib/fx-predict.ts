...
export const computeFeatures = (dbPath: string, pair: string, date: string): Features | null => {
  ...
  try {
    ...
    for (let back = 1; back <= 7; back++) {
      ...
      const pRow = db
        ...
        .get(pair, jstDateToEntryUtcSec(py, pm, pd));
      if (pRow) {
        prev = pRow.o;
        break;
      }
...
export const predict = async (opts: {
  ...
  const dbPath = opts.dbPath ?? findLocalDb();
  const f = computeFeatures(dbPath, opts.pair, opts.date);
